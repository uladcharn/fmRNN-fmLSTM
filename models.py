...
class FMRNN(nn.Module):
    """f-mRNN with a dynamic d for time series prediction"""
    def __init__(self, input_size, hidden_size, output_size, k, n_banks, bias=True):
        super(FMRNN, self).__init__()
        self.k = k
        self.input_size = input_size
        self.output_size = output_size
        self.fmrnn_cell = layers.FMRNNCell(input_size, hidden_size,
                                         output_size, k, n_banks)
    ...
    def forward(self, inputs, hidden_state=None):
        ...
        outputs = torch.zeros(time_steps, batch_size, self.output_size,
                              dtype=inputs.dtype, device=inputs.device)
        for times in range(time_steps):
            outputs[times, :], hidden_state = self.fmrnn_cell(inputs[times, :],
                                                             hidden_state)
        return outputs, hidden_state
    
class FMLSTM(nn.Module): 
    """f-mLSTM with a dynamic d for time series prediction"""
    def __init__(self, input_size, hidden_size, k, n_banks, output_size):
        ...
        self.k = k
        self.output_size = output_size
        self.fmlstm_cell = layers.FMLSTMCell(self.input_size, self.hidden_size,
                                           self.k, n_banks, self.output_size)
    ...
    def forward(self, inputs, hidden_state=None):
        ...
        outputs = torch.zeros(time_steps, batch_size, self.output_size,
                              dtype=inputs.dtype, device=inputs.device)
        for times in range(time_steps):
            outputs[times, :], hidden, h_list, d_values = \
                self.fmlstm_cell(inputs[times, :], hidden, h_list, d_values)
        return outputs, (hidden, h_list, d_values)
    
class FMRNNFixD(nn.Module): 
    """f-mRNN-mm with a fixed d for time series prediction"""
    def __init__(self, input_size, hidden_size, output_size, k, n_banks, bias=True):
        super(FMRNNFixD, self).__init__()
        self.k = k
        self.input_size = input_size
        self.output_size = output_size
        self.b_d = [Parameter(torch.Tensor(torch.zeros(1, input_size)),
                             requires_grad=True) for _ in range(n_banks)]
        self.fmrnn_cell = layers.FMRNNFixDCell(input_size, hidden_size,
                                             output_size, k, n_banks)
        self.filter_bank_params = {
            'd_weights': []
        }
        self.n_banks = n_banks
        for i in range(self.n_banks):
            self.d_matrix = 0.5 * F.sigmoid(self.b_d[i].clone().detach())
            weights_d = self.get_wd(self.d_matrix)
            self.filter_bank_params['d_weights'].append(weights_d)
    ...
    def forward(self, inputs, hidden_state=None):
        ...
        outputs = torch.zeros(time_steps, batch_size, self.output_size,
                              dtype=inputs.dtype, device=inputs.device)
        for times in range(time_steps):
            outputs[times, :], hidden_state = self.fmrnn_cell(inputs[times, :], self.filter_bank_params,
                                                   hidden_state)
        return outputs, hidden_state
    
class FMLSTMFixD(nn.Module): 
    """f-mLSTM with a fixed d for time series prediction"""
    def __init__(self, input_size, hidden_size, k, output_size, n_banks):
        super(FMLSTMFixD, self).__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.k = k
        self.n_banks = n_banks
        self.d_values = [Parameter(torch.Tensor(torch.zeros(1, hidden_size)),
                                  requires_grad=True) for _ in range(n_banks)]
        self.output_size = output_size
        self.fmlstm_cell = layers.FMLSTMFixDCell(self.input_size,
                                               self.hidden_size,
                                               self.output_size,
                                               self.k, self.n_banks)
        self.sigmoid = nn.Sigmoid()
        ...
        self.n_banks = n_banks
        for i in range(self.n_banks):
            self.d_values_sigmoid = 0.5 * F.sigmoid(self.d_values[i].clone().detach())
            weights_d = self.get_wd(self.d_values_sigmoid)
            self.filter_bank_params['d_weights'].append(weights_d)
    ...
    def forward(self, inputs, hidden_states=None):
        ...
        for times in range(time_steps):
            outputs[times, :], hidden, h_list = self.fmlstm_cell(inputs[times, :],
                                                             hidden,
                                                             h_list,
                                                             self.filter_bank_params)
        return outputs, (hidden, h_list)