...
PARSER.add_argument('--n_banks', type=int, default=5,
...
def main():
    ...
    for i in range(start, end):
        ...
        if FLAGS.algorithm == 'RNN':
            ...
            model = models.MLSTM(input_size=FLAGS.input_size,
                                 ...
                                 output_size=FLAGS.output_size,
                                 k=FLAGS.K)
        elif FLAGS.algorithm == 'fmRNN':
            model = models.FMRNN(input_size=FLAGS.input_size,
                                hidden_size=FLAGS.hidden_size,
                                output_size=FLAGS.output_size,
                                k=FLAGS.K, n_banks=FLAGS.n_banks)
        elif FLAGS.algorithm == 'fmRNN_fixD':
            model = models.FMRNNFixD(input_size=FLAGS.input_size,
                                hidden_size=FLAGS.hidden_size,
                                output_size=FLAGS.output_size,
                                k=FLAGS.K, n_banks=FLAGS.n_banks)
        elif FLAGS.algorithm == 'fmLSTM':
            model = models.FMLSTM(input_size=FLAGS.input_size,
                                 hidden_size=FLAGS.hidden_size,
                                 output_size=FLAGS.output_size,
                                 k=FLAGS.K, n_banks=FLAGS.n_banks)
        elif FLAGS.algorithm == 'fmLSTM_fixD':
            model = models.FMLSTMFixD(input_size=FLAGS.input_size,
                                 hidden_size=FLAGS.hidden_size,
                                 output_size=FLAGS.output_size,
                                 k=FLAGS.K, n_banks=FLAGS.n_banks)
        else:
    ...
    print("Mean - RMSE: " f"{np.mean(rmse_list)}" " Standard Deviation - RMSE: " f"{np.std(rmse_list)}")
    print("Mean - MAE: " f"{np.mean(mae_list)}" " Standard Deviation - MAE: " f"{np.std(mae_list)}")
    print("Best RMSE: " f"{min(rmse_list)}")
    print("Best MAE: " f"{min(mae_list)}")
