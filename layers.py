...
########################## Filter-banked Section ##########################
    
class FMRNNFixDCell(nn.RNNCellBase): 
    """filter bank-embedded memory augmented RNN with fixed D with multiple memory filters"""
    def __init__(self, input_size, hidden_size, output_size, k, n_banks, bias=True):
        super(FMRNNFixDCell, self).__init__(input_size, hidden_size, bias,
                                           num_chunks=1)
        self.k = k
        ...
        self.hidden_size = hidden_size
        self.output_size = output_size
        self.f_m_layers = nn.ParameterList([nn.Linear(input_size, hidden_size, bias=False) for _ in range(n_banks)])
        self.m_m = nn.Linear(hidden_size, hidden_size, bias=True)
    ...
    def forward(self, inputs, filter_bank_params, hidden_state=None):
        if hidden_state is None:
            h_0 = torch.zeros(inputs.size(0), self.hidden_size,
                              dtype=inputs.dtype, device=inputs.device)
            m_0 = torch.zeros(inputs.size(0), self.hidden_size,
        ...
        x_filter_sum = torch.zeros(inputs.size(0), self.hidden_size,
                              dtype=inputs.dtype, device=inputs.device)

        for i in range(self.n_banks):
            x_filter = torch.einsum('ijk,ik->ijk', [x_combine, filter_bank_params['d_weights'][i]]).sum(dim=0) # consider only updating the filter

            x_filter_sum += self.layer_norm(self.f_m_layers[i](x_filter))

        mem = F.tanh(self.m_m(m_0) + x_filter_sum)
...
class FMLSTMFixDCell(nn.Module):
    """filter bank-embedded memory augmented LSTM with fixed D"""
    def __init__(self, input_size, hidden_size, output_size, k, n_banks):
        super(FMLSTMFixDCell, self).__init__()
        self.hidden_size = hidden_size
        self.k = k
        self.n_banks = n_banks
        self.output_size = output_size
        ...
        self.i_gate = nn.Linear(input_size + hidden_size, hidden_size)
        self.o_gate = nn.Linear(input_size + hidden_size, hidden_size)
        self.first_weights = nn.ParameterList([nn.Linear(hidden_size, hidden_size, bias=False) for _ in range(n_banks)])
        self.output = nn.Linear(hidden_size, output_size)
        ...
        self.layer_norm = nn.LayerNorm(hidden_size, hidden_size)

    def forward(self, sample, hidden, cell_tensor, filter_bank_params):
        batch_size = sample.size(0)
        if hidden is None:
            hidden = torch.zeros(batch_size, self.hidden_size,
                                 dtype=sample.dtype, device=sample.device)
        ...
        first_sum = torch.zeros(batch_size, self.hidden_size,
                                 dtype=sample.dtype, device=sample.device)
        ...
        for i in range(self.n_banks): # consider removing different i- and c-tilde-gates, leaving only filter update
            first = torch.einsum('ijk,ik->ijk', [-cell_tensor, filter_bank_params['d_weights'][i]]).sum(dim=0)
            first = self.first_weights[i](first)
            first_sum += self.layer_norm(first) 

        cell = torch.add(first_sum,second) 
...
class FMRNNCell(nn.RNNCellBase): 
    """filter bank-embedded memory augmented RNN with dynamic D"""
    def __init__(self, input_size, hidden_size, output_size, k, n_banks, bias=True):
        super(FMRNNCell, self).__init__(input_size, hidden_size, bias,
                                       num_chunks=1)  
        self.k = k
        self.n_banks = n_banks
        self.input_size = input_size
        ...
        self.d_layers = [nn.Linear(input_size, input_size, bias=False) for _ in range(n_banks-1)] 
        self.d_layers.append(nn.Linear(input_size, input_size, bias=True))
        self.d_layers = nn.ParameterList(self.d_layers)
        self.h_d = nn.ParameterList([nn.Linear(hidden_size, input_size, bias=False) for _ in range(n_banks)])
        self.m_d_layers = nn.ParameterList([nn.Linear(hidden_size, input_size, bias=False) for _ in range(n_banks)])
        self.x_d = nn.ParameterList([nn.Linear(input_size, input_size, bias=False) for _ in range(n_banks)])
    ...
    def forward(self, inputs, hidden_state=None):
        if hidden_state is None:
            ...
            m_0 = torch.zeros(inputs.size(0), self.hidden_size,
                              dtype=inputs.dtype, device=inputs.device)
            d_0 = nn.ParameterList([torch.zeros(inputs.size(0), self.input_size,
                              dtype=inputs.dtype, device=inputs.device) for _ in range(self.n_banks)])
            x_s = torch.zeros(self.k - 1, inputs.size(0), self.input_size,
                             dtype=inputs.dtype, device=inputs.device)
            hidden_state = (h_0, m_0, d_0, x_s)
        ...
        for i in range(self.n_banks):
            d_values = 0.5 * F.sigmoid(self.d_layers[i](d_0[i]) + self.h_d[i](h_0) +
                                   self.m_d_layers[i](m_0) + self.x_d[i](inputs)) 
        ...
        for i in range(self.n_banks):
            x_filter = self.filter_d(x_combine, d_values_list[i])
            x_filter_sum += self.f_m_layers[i](x_filter) 
        ...
        z_out = self.h_z(hid) + self.m_z(mem)
        xs_out = x_combine[1:, :]
        return z_out, (hid, mem, d_values_list, xs_out)
    
class FMLSTMCell(nn.Module): # think about i-gate, c-tilde-gate, and o-gate
    """filter bank-embedded memory augmented LSTM with dynamic D"""
    def __init__(self, input_size, hidden_size, k, n_banks, output_size):
        super(FMLSTMCell, self).__init__()
        self.hidden_size = hidden_size
        self.k = k
        self.n_banks = n_banks
        self.output_size = output_size
        ...
        self.c_gate = nn.Linear(input_size + hidden_size, hidden_size)
        self.i_gate = nn.Linear(input_size + hidden_size, hidden_size)
        self.f_gate_layers = nn.ParameterList([nn.Linear(input_size + 2 * hidden_size, hidden_size) for _ in range(n_banks)])
        self.o_gate = nn.Linear(input_size + hidden_size, hidden_size)
        self.first_weights = nn.ParameterList([nn.Linear(hidden_size, hidden_size, bias=False) for _ in range(n_banks)])
        self.output = nn.Linear(hidden_size, output_size)
    ...
    def forward(self, sample, hidden, cell_tensor, d_0):
        ...
        if cell_tensor is None:
            cell_tensor = torch.zeros(self.k, batch_size, self.hidden_size,
                                      dtype=sample.dtype, device=sample.device)
        if d_0 is None:
            d_0 = nn.ParameterList([torch.zeros(batch_size, self.hidden_size,
                              dtype=sample.dtype, device=sample.device) for _ in range(self.n_banks)])

        combined = torch.cat((sample, hidden), 1)
        
        d_values_list = []
        for i in range(self.n_banks):
            combined_d = torch.cat((sample, hidden, d_0[i]), 1) 
            d_values = self.f_gate_layers[i](combined_d)
            d_values = self.sigmoid(d_values) * 0.5
            d_values_list.append(d_values)
        ...
        for i in range(self.n_banks): 
            first = torch.einsum('ijk,ik->ijk', [-cell_tensor, d_values_list[i]]).sum(dim=0)
            first = self.first_weights[i](first)
            first_sum += self.layer_norm(first)
        ...
        hidden = torch.mul(self.tanh(cell), o_gate)
        output = self.output(hidden)
        return output, hidden, h_c_1, d_values_list
